...
- Success-rate change: `+0.0 pp` (baseline `100.0%`, untrained `100.0%`, trained `100.0%`).
- Utility trade-off: `-0.032` (small decrease can occur when the policy closes deals earlier).
...
### Composite Metric
- `Efficiency Score = Avg Reward / Avg Rounds` (reported in table).
